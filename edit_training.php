	...
	else
	{
		...
		while ($row = $result -> fetch_assoc()) 
		{
			...
			echo "<tr style = \"vertical-align: middle;\">
			      ...
			      <td style=\"vertical-align:middle;\"> <a href=\"edit_training_data.php?nr=".$nr."&id_session=".$id_session."\" class = \"btn btn-primary\">Edit training</a> </td>";
				...
				<?php
					...
					try 
					{
						...
						while ($row = $result -> fetch_assoc())
						{
							...
							if($permanent == 1 || $id_user_exercise == $id_user)
			...
			try 
			{
				...
				while ($row = $result -> fetch_assoc())
				{
					...
					echo "<li class=\"list-group-item li-training justify-content-between\" style = \"text-align:left;\"> <a style=\"margin-left:4px;\" href = \"delete_exercise_from_training.php?id_session=".$id_session."&id_series=".$id_series."&nr=".$nr."\" class=\"close\"> &times;</a>".$name;
							...
							if($repetition != NULL)
							{
								echo "<span class=\"badge badge-default badge-pill badge-edit\">".$repetition."</span>";
							}
